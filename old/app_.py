...
with st.sidebar:
    ...
    qiq.handle_logo()
    st.title("QUANT-IQ")
    selected = option_menu("Main Menu", ["Analyze", "Prompt", "Settings"],
                           icons=['magic', "list-task", "gear"], menu_icon="cast", default_index=0)

    st.session_state.assistant_mode = st.radio("Assistant Mode", [
                                               "On", "Off"],
                                               index=1,
...
if selected == "Settings":
    ...
    if st.session_state.key_set:
        if st.button("set Keys", key="add_keys", help="Set keys for use.", use_container_width=True):
    ...
    with st.container():
        ...
        if not st.session_state["bulk_file_uploaded"]:
            ...
            if st.session_state["files"]:
                ...
                logging.info(
                    "All files uploaded and session state updated.")
                st.rerun()
...
if selected == "Prompt":
    ...
    selected2 = option_menu(None, ["Edit", "Save", "Download", 'Restore'],
                            icons=["list-task", "robot", 'cloud-download',
                                   'reply'],
    ...
    if selected2 == "Edit":
        with st.expander("Prompt", expanded=True):
            ...
            with st.container():
                current_content = st_quill(toolbar=[['bold', 'italic', 'underline'],
                                                    ['link', 'blockquote',
                                                     'code-block']],
    ...
    if selected2 == "Save":
        if st.session_state.assistant_instructions != st.session_state.editor_content+pu.get_output_format():
            ...
            if st.session_state.assistant_mode == "On":
                ...
                st.session_state.assistant_instructions = st.session_state.editor_content + \
